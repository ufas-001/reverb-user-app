...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function (event) {
       if (event.origin === "http://localhost:3000") {
         ...
         if (message && message.developerKey) {
           console.log("Received developer key:", message.developerKey);
  ...
  // function getDevKey () {
  //     const developerKey = "3738d7d0-ba8c-4ff8-a4da-ea0a9f1ea498";
  //       if (developerKey) {
  //         console.log("Received developer key:", developerKey);
  //         checkCSS(developerKey);
  //       }
  // }
  // getDevKey()
});
...
function initializeWidget(developerKey) {
  ...
  socket.on("messageCreated", () => {
    console.log("Message received from server. Updating messages display...");
    displayMessages("socket"); // Ensure this line is only executed once per event
  });
  ...
  showWidget.addEventListener("click", function () {
    ...
    window.parent.document.querySelector("iframe").style.height = "100%";
    window.parent.document.querySelector("iframe").style.width = "460px";
  });
  ...
  hideWidget.addEventListener("click", () => {
    ...
    window.parent.document.querySelector("iframe").style.height = "80px";
    window.parent.document.querySelector("iframe").style.width = "200px";
  });

  dropWidget.addEventListener("click", () => {
    showWidget.style.display = "block";
    hideWidget.style.display = "none";
    widgetContainer.style.display = "none";
    messageDiv.style.display = "none";
  ...
  async function displayMessages(when) {
    console.log("displayMessages called");
    // Ensure chatMessages is correctly selected
    const chatMessages = document.getElementById("chat-messages");
    if (!chatMessages) {
    ...
    try {
      ...
      reverseMessage.forEach((msg) => {
        ...
        //     <div class="px-4 py-2 rounded-lg rounded-br-none relative bg-gray-200 h-9 w-24 flex items-center">
        //       <div className="typing__dot bg-gray-900"></div>
        //       <div className="typing__dot bg-gray-900"></div>
        //       <div className="typing__dot bg-gray-900"></div>
        //     </div>
        //   `;
  ...
  const init = async () => {
    await getPreferenceColor();
    await fetchAndDisplayArticles();
    await displayMessages("init");
